...
. /opt/app-root/scripts/common/libfile.sh
. /opt/app-root/scripts/common/liblog.sh
. /opt/app-root/scripts/common/libnet.sh
. /opt/app-root/scripts/common/libos.sh
. /opt/app-root/scripts/common/libservice.sh
. /opt/app-root/scripts/common/libvalidations.sh
. /opt/app-root/scripts/common/libpersistence.sh
...
airflow_scheduler_initialize() {
    ...
    [[ "$AIRFLOW_EXECUTOR" == "CeleryExecutor" || "$AIRFLOW_EXECUTOR" == "CeleryKubernetesExecutor"  ]] && /opt/app-root/scripts/common/wait-for-it.sh "$REDIS_HOST":"$REDIS_PORT_NUMBER" --timeout=30 --quiet
...
airflow_scheduler_wait_for_webserver() {
    ...
    check_webserver_connection() {
        /opt/app-root/scripts/common/wait-for-it.sh "$webserver_host":"$webserver_port" --timeout=30 --quiet
    }
