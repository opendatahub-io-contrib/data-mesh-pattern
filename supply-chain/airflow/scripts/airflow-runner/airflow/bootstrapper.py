...
class FileOpBase(ABC):
    ...
    def process_dependencies(self) -> None:
        ...
        try:
            archive_dir_name = archive_file.replace(".tar.gz", "")
            print("archive folder name is ", archive_dir_name)
            subprocess.check_call([sys.executable, "-m", "pip", "install", "-r", "base_requirements.txt"])
        except Exception as e:
            print(e.output)
        OpUtil.log_operation_info("pip install done", duration)
    ...
    def get_file_from_object_storage(self, file_to_get: str) -> None:
        ...
        try:
            ...
            print("WARNING: Could not download %s - %s" %(file_to_get, err))
        duration = time.time() - t0
