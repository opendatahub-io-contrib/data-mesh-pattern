...
web_server_execute() {
    ...
    (
        . "/opt/app-root/scripts/lib${web_server}.sh"
        . "/opt/app-root/scripts/${web_server}-env.sh"
        "$@"
    )
}
...
web_server_list() {
    ...
    for web_server in "${supported_web_servers[@]}"; do
        [[ -f "/opt/app-root/scripts/${web_server}-env.sh" ]] && existing_web_servers+=("$web_server")
    done
...
web_server_validate() {
    ...
    if [[ -z "$(web_server_type)" || ! " ${supported_web_servers[*]} " == *" $(web_server_type) "* ]]; then
        ...
        print_validation_error "Could not load the $(web_server_type) web server library from /opt/app-root/scripts. Check that it exists and is readable."
    fi
...
web_server_start() {
    ...
    "${APP_ROOT}/scripts/$(web_server_type)/start.sh"
}
...
web_server_stop() {
    ...
    "${APP_ROOT}/scripts/$(web_server_type)/stop.sh"
}
...
web_server_restart() {
    ...
    "${APP_ROOT}/scripts/$(web_server_type)/restart.sh"
}
...
#########################
web_server_reload() {
    "${APP_ROOT}/scripts/$(web_server_type)/reload.sh"
}
